...
export function SalesFormPage() {
    ...
    useEffect(() => {
        ...
        if (!isEdit) return
            ; (async () => {
                ...
                await Promise.all(resourceIds.map((rid) => loadCircuitsForResource(rid)))
            })().catch((err) => {
                console.error(err)
    ...
    const addItem = () => setItems((prev) => [...prev, createEmptyItemDraft()])
    const removeItem = (uiId: string) => setItems((prev) => prev.filter((item) => item.ui_id !== uiId))

    const toggleCircuit = (uiId: string, circuitId: string) => {
    ...
    const updateItem = (uiId: string, field: keyof ItemDraft, value: string) => {
        setItems((prev) => prev.map((item) => {
            if (item.ui_id !== uiId) return item
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isEdit) {
                ...
                const order = await createSalesOrder({ internal_ref: internalRef || undefined, customer_id: customerId, status, notes })
        ...
        <div className="max-w-4xl mx-auto">
            ...
            {step === 2 && (
                <div className="space-y-4">
                    ...
                    {items.map((item, idx) => (
                        <div key={item.ui_id} className="bg-surface rounded-xl border border-border-subtle p-5 space-y-4">
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-text">Item {idx + 1}</span>
                                <button onClick={() => removeItem(item.ui_id)} className="p-1.5 rounded-md hover:bg-destructive/10 text-text-dim hover:text-destructive transition-colors cursor-pointer">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <select value={item.type} onChange={(e) => updateItem(item.ui_id, 'type', e.target.value)}
                                        className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                    >
                                ...
                                {(FIELD_CFG[item.type]?.disposal ?? false) && (
                                    <div>
                                        ...
                                        <select value={item.disposal_type} onChange={(e) => updateItem(item.ui_id, 'disposal_type', e.target.value)}
                                            className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                        >
                                ...
                                {canLinkInventory(item.type) ? (
                                    <div>
                                        ...
                                        <SearchableSelect
                                            ...
                                            onChange={(v) => {
                                                ...
                                                setItems(prev => prev.map((it) => it.ui_id === item.ui_id ? {
                                                    ...it,
                                    ...
                                    <div className={!FIELD_CFG[item.type]?.disposal ? 'col-span-2' : ''}>
                                        ...
                                        <input type="text" value={item.description} onChange={(e) => updateItem(item.ui_id, 'description', e.target.value)}
                                            ...
                                            {circuits.map((c) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        onClick={() => toggleCircuit(item.ui_id, c.id)}
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                {FIELD_CFG[item.type]?.capacity && (<>
                                    <div>
                                        ...
                                        <input type="number" value={item.capacity} onChange={(e) => updateItem(item.ui_id, 'capacity', e.target.value)}
                                            className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs text-text-dim mb-1">Spec</label>
                                        <input type="text" value={item.spec} onChange={(e) => updateItem(item.ui_id, 'spec', e.target.value)} placeholder="100G"
                                            className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text placeholder:text-text-dim focus:ring-1 focus:ring-primary focus:outline-none"
                                        />
                                    </div>
                                </>)}
                                <div>
                                    ...
                                    <input type="date" value={item.start_date} onChange={(e) => updateItem(item.ui_id, 'start_date', e.target.value)}
                                        className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                    />
                                </div>
                                {FIELD_CFG[item.type]?.term && (
                                    <div>
                                        ...
                                        <input type="number" value={item.term_months} onChange={(e) => updateItem(item.ui_id, 'term_months', e.target.value)}
                                            className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                        />
                                    </div>
                                )}
                                ...
                                {FIELD_CFG[item.type]?.disposal && isIRUStyle(item.disposal_type) ? (
                                    <>
                                        <div>
                                            ...
                                            <input type="number" value={item.sell_otc} onChange={(e) => updateItem(item.ui_id, 'sell_otc', e.target.value)}
                                                className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs text-text-dim mb-1">O&M Rate (%)</label>
                                            <input type="number" step="0.1" value={item.sell_om_rate} onChange={(e) => updateItem(item.ui_id, 'sell_om_rate', e.target.value)}
                                                className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs text-text-dim mb-1">Annual O&M ($)</label>
                                            <input type="number" value={item.sell_annual_om} onChange={(e) => updateItem(item.ui_id, 'sell_annual_om', e.target.value)}
                                                className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs text-text-dim mb-1">NRC ($)</label>
                                            <input type="number" value={item.sell_nrc} onChange={(e) => updateItem(item.ui_id, 'sell_nrc', e.target.value)}
                                                className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                            />
                                        </div>
                                    </>
                                    ...
                                    <>
                                        {FIELD_CFG[item.type]?.mrc && (
                                            <div>
                                                ...
                                                <input type="number" value={item.sell_mrc} onChange={(e) => updateItem(item.ui_id, 'sell_mrc', e.target.value)}
                                                    className="w-full px-3 py-2 bg-background border border-border-subtle rounded-lg text-sm text-text focus:ring-1 focus:ring-primary focus:outline-none"
                                                />
                                            </div>
                                        )}
